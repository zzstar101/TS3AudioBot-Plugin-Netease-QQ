...
测试过在Docker环境下能正常使用，Windows和Linux同样支持。   

插件前端地址（Web 控制台）：https://github.com/zzstar101/ts3audiobot-frontend

> 说明：本仓库仅维护插件代码与配置，不再内置 Dockerfile 或架构测试脚本。容器化部署请参考下文外部项目链接。   

## 分支差异分析（远端非 main）

本节用于记录“远端非 main 分支”与“本地同名分支”的完整差异检查结果。

- 分析时间：2026-03-28
- 远端仓库：https://github.com/zzstar101/TS3AudioBot-Plugin-Netease-QQ
- 分析分支（非 main）：`origin/sync-from-toolarea-20260314`
- 本地分支：`sync-from-toolarea-20260314`

检查口径：

- `git rev-list --left-right --count origin/sync-from-toolarea-20260314...HEAD`
- `git diff --name-status origin/sync-from-toolarea-20260314...HEAD`
- `git diff --stat origin/sync-from-toolarea-20260314...HEAD`
- `git log --left-right --oneline origin/sync-from-toolarea-20260314...HEAD`
- `git status --short`
...
结论：当前本地分支与远端非 main 分支已完全同步。

最近一次已同步功能提交（同分支）：`2af5667`

- 文件范围：`TS3AudioBot_Plugin_Netease_QQ.cs`、`MusicAPI.cs`、`README.md`
- 主要内容：
  - 新增 `wq resume` 命令与暂停恢复重建逻辑，降低长暂停后误切下一首的概率。
  - 在 `PlaybackStopped` 事件中加入异常停止恢复分支，先尝试恢复当前曲目再决定切歌。
  - 为 HTTP 请求增加超时处理，减少恢复阶段请求卡死造成的连锁失败。
  - 文档补充前端地址与权限项（`cmd.wq.resume`）。
...
说明：当前仓库不提供 Dockerfile，请使用下方推荐项目部署 TS3AudioBot 与相关 API 容器。
